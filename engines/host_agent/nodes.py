...
class HostNodes:
    ...
    async def generate_section_judgement(
        ...
        logger.info(f"【{role_display_name('host')}】双Agent章节已齐备,开始研判 章节={section_pair.section_key}")
    ...
    async def apply_section_judgement(
        ...
        logger.info(f"【{role_display_name('host')}】章节研判完成 章节={section_judgement.section_key}")
        return {
            "section_pair_store": state["section_pair_store"],
